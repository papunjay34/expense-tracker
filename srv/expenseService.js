...
module.exports = cds.service.impl(async function () {
    ...
    this.before('CREATE', 'Expenses', req => {
        ...
        if (!date)
            return req.error(400, 'Expense date is required');
    });
    ...
    this.before('CREATE', 'Budgets', req => {
        ...
        if (!startDate)
            return req.error(400, 'Budget start date is required');
        if (!endDate)
            return req.error(400, 'Budget end date is required');
    ...
    this.before('CREATE', 'Goals', req => {
        ...
        if (!goalName?.trim())
            return req.error(400, 'Goal name is required');
    ...
    this.on('getBudgetUtilisation', async req => {
        ...
        if (!userId)
            return req.error(400, 'userId is required');
        if (!categoryId)
            return req.error(400, 'categoryId is required');
        ...
        const utilisation = budget.amount > 0
